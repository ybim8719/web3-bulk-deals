...
contract NftLuckyDipTest is Test {
    ...
    string constant ADDITIONNAL_MOCK_DESCRIPTION =
        "description of lucky Dip #2";
    ...
    modifier oneBidWasMade() {
        ...
        assertEq(s_luckyDip.isLuckyDipPublished(0), true);
        vm.startPrank(user1);
        s_luckyDip.bidForLuckyDip{
            value: s_luckyDip.getNextBiddingPriceInWei(0)
        }(0);
        vm.stopPrank();
    ...
    function testOwnerCanAddLuckyDip() public {
        ...
        assertEq(
            s_luckyDip.getNbOfLuckyDips(),
            ADDITIONNAL_MOCK_NB_OF_LUCKY_DIPS
        );
        assertEq(
            s_luckyDip.getLuckyDipNFTLength(1),
            ADDITIONNAL_MOCK_IMAGE_URI_LENGTH
        );
        assertEq(
            s_luckyDip.getLuckyDipDescription(1),
            ADDITIONNAL_MOCK_DESCRIPTION
        );
        assertEq(s_luckyDip.getStartingBid(1), ADDITIONNAL_MOCK_STARTINGBID);
        assertEq(
            s_luckyDip.getNextBiddingPriceInWei(1),
            ADDITIONNAL_MOCK_STARTINGBID
        );
